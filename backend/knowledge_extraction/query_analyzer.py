...
try:
    ...
    logging.critical("WARNING: Could not import llm_integration from backend.")
    DEFAULT_TIMEOUT = 30
    class GeorgeAI: # Dummy class
        def chat(self, *args, **kwargs):
             return {'success': False, 'response': None, 'error': "LLM Integration module failed to load."}

# Try to import parsers - may not be available
try:
    from parsers.parsers import read_manuscript_file 
except ImportError:
    logging.warning("Could not import read_manuscript_file from parsers module. Using file read fallback.")
    def read_manuscript_file(filepath: str) -> str:
        """Simple file reading fallback"""
        try:
            with open(filepath, 'r', encoding='utf-8') as f:
                return f.read()
        except Exception as e:
            raise ImportError(f"Could not read file {filepath}: {e}")
...
class QueryAnalyzer:
    ...
    def __init__(self,
                 ...
                 app_help_path: str = "backend/app_help_docs",
