...
# This ensures Firebase is initialized (ideally this is called in app.py)
if not firebase_admin._apps:
    ...
    try:
        ...
        credential_path_options = [
            ...
            base_dir / "instance" / "service-account-key.json"
        ]
        ...
        if CREDENTIALS_PATH:
            ...
            logger.critical("Firebase service account key not found. Authentication will fail.")
            
    except Exception as init_e:
        logger.critical(f"Failed to initialize Firebase Admin SDK in auth_client: {init_e}", exc_info=True)

# --- UPDATED: Function to get or create customer document ---
def get_or_create_customer_profile(user_id: str, email: str) -> dict:
    ...
    with a default "Free" tier if it doesn't exist.
    """
    db = firestore.client() # Get the client from the initialized app
    ...
    try:
        ...
        if doc.exists:
            ...
            logger.info(f"No customer profile found for user: {user_id}. Creating default 'Free' profile.")
            
            new_customer_data = {
                'email': email,
                'subscriptionTier': 'Free',        # Default to Free tier
                'subscriptionStatus': 'active',      # Start as active
                'apiPoolBalance': 0.00,              # Start with a $0.00 API pool
                'paymentProviderCustomerID': None,   # To be filled by Stripe Extension
        ...
        # Return a non-functional default or raise the exception
        raise

def verify_firebase_token():
    """
    Decorator to protect Flask routes by verifying the Firebase ID token
    AND loading the customer profile.
    """
    def decorator(f):
        @wraps(f)
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                # --- Step 2: Get or create the customer profile ---
                ...
                if 'text/html' in request.accept_mimetypes:
                    flash(f"Authentication failed ({e}). Please log in again.", "error")
                    return redirect(url_for('auth.login', next=request.url))
                else:
